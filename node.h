...
public:
    ...
    NAssignment(NIdentifier &lhs, NExpression &rhs)
        : lhs(lhs), rhs(rhs), isArrayVariable(false), variable(lhs, rhs) {
        line = lineNumber;
    }
    NAssignment(NIdentifier &lhs, NExpression &rhs, NExpression &index)
        : lhs(lhs), rhs(rhs), isArrayVariable(true), variable(lhs, index) {
        line = lineNumber;
    }
    virtual void codeGen(CodeGenContext &context);
};
...
public:
    ...
    NVariableDeclaration(ExprType type, NIdentifier &id)
        : type(type), id(id), assignmentExpr(nullptr) {
        line = lineNumber;
    }
    NVariableDeclaration(ExprType type, NIdentifier &id, NExpression *assignmentExpr)
...
public:
    ...
    NBlock *block;
    NForStatement(NExpression *initializerExpr, NExpression *condition,
                  NExpression *iterator, NBlock *block)
        : initializerExpr(initializerExpr), initializerDecl(nullptr),
          condition(condition), iterator(iterator), block(block) {
        line = lineNumber;
    }
    NForStatement(NStatement *initializerDecl, NExpression *condition,
                  NExpression *iterator, NBlock *block)
        : initializerExpr(nullptr), initializerDecl(initializerDecl),
          condition(condition), iterator(iterator), block(block) {
        line = lineNumber;
    }
    virtual void codeGen(CodeGenContext &context);
...
public:
    ...
    NArrayDeclaration(ExprType type, NIdentifier &id, NExpression *lengthExpr)
        : type(type), id(id), lengthExpr(lengthExpr) {
        line = lineNumber;
    }
    virtual void codeGen(CodeGenContext &context);
};
